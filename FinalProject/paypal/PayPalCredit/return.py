...
if  token != "":
	...
	*'''
	...
	if ackGetExpressCheckout == "SUCCESS" or ackGetExpressCheckout == "SUCESSWITHWARNING":		
		...
		*/'''
		#if _SESSION.get("Payment_Amount") != totalAmt or _SESSION.get("currencyCodeType") != currencyCode :
...
if USERACTION_FLAG=='False' and form.getvalue('shipping_method'):
	if form.getvalue('shipping_method'):
		...
		if new_shipping == 'ups_XPD':
			shipping_amt = 25.43
		elif new_shipping == 'ups_WXS':
			shipping_amt = 15.67
		elif new_shipping == 'flatrate_flatrate':
			shipping_amt = 5.00
		else:
			shipping_amt = shippingAmt		
	if shippingAmt > 0:
		finalPaymentAmount = float(totalAmt) + float(shipping_amt) - float(shippingAmt)
'''/* Review block end */
/*
